...
const processSubmission = async (userId, problemId, isCorrect, problemData) => {
    ...
    try {
       ...
       FROM users WHERE id = $1 FOR UPDATE`, [userId]);
        ...
        const today = (0, gameSystemService_1.getTodayString)();
        perfMark('userSelect');
        // 3. Daily reset (in-memory, replaces handleDailyReset)
        let quests = Array.isArray(u.quests) ? u.quests : [];
        if (u.last_active_date !== today) {
            quests = (0, gameSystemService_1.generateDailyQuests)();
        }
        ...
        if (u.last_active_date && u.last_active_date !== today) {
            const diff = (0, gameSystemService_1.getDaysDifference)(u.last_active_date, today);
            if (diff >= 2) {
        ...
        if (isCorrect && dbLastActiveDate !== today) {
            const diff = (0, gameSystemService_1.getDaysDifference)(dbLastActiveDate || '', today);
        ...
        const processQuestAction = (action, data) => {
            quests = quests.map((q) => {
                if (q.completed)
                    return q;
                const updatedQ = { ...q };
