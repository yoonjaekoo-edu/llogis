...
exports.recordFingerprint = recordFingerprint;
// ─── 일회용 이메일 도메인 블랙리스트 ───
...
function generateServerFingerprint(userAgent, acceptLanguage) {
...
// ─── 어뷰징 체크 (DB 쿼리) ───
async function checkAbuse(pool, params) {
    ...
    const fpResult = await pool.query(`SELECT COUNT(*) as count FROM signup_fingerprints
     WHERE visitor_id = $1
     AND created_at > NOW() - INTERVAL '24 hours'`, [fingerprint]);
    ...
    const ipResult = await pool.query(`SELECT COUNT(*) as count FROM signup_fingerprints
     WHERE ip_subnet = $1
     AND created_at > NOW() - INTERVAL '24 hours'`, [ipSubnet]);
...
// ─── 핑거프린트 기록 ───
async function recordFingerprint(pool, params) {
    const { visitorId, ipSubnet, userAgent, userId } = params;
    await pool.query(`INSERT INTO signup_fingerprints (visitor_id, ip_subnet, user_agent, user_id)
     VALUES ($1, $2, $3, $4)`, [visitorId, ipSubnet, userAgent, userId]);
}
